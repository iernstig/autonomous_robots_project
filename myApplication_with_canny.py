...
def findCones(input_img, low1, high1, low2, high2, hsv_img, image, cid):
  im2, cnts, hier = cv2.findContours(input_img.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
  blue_hits = []
  yellow_hits = []
  print("Number of hits tot: " + str(len(cnts)))
  for c in cnts:
    ...
    if (area < 2000 and area > 0):
      ...
      cX = int(M["m10"] / M["m00"])
      cY = int(M["m01"] / M["m00"])
      if(low1[0] <= hsv_img[cY, cX, 0] <= high1[0] and low1[1] <= hsv_img[cY, cX, 1] <= high1[1]
        and low1[2] <= hsv_img[cY, cX, 2] <= high1[2]):
      ...
      elif (low2[0] <= hsv_img[cY, cX, 0] <= high2[0] and low2[1] <= hsv_img[cY, cX, 1] <= high2[1]
        and low2[2] <= hsv_img[cY, cX, 2] <= high2[2]):
      #else:
    ...
    if (cid == 253):
      for i in range(len(blue_hits)):
        cX = blue_hits[i][0]
        cY = blue_hits[i][1]
        #cv2.drawContours(image, [c], 0, (255, 0, 0), 2)
        cv2.circle(image, (cX, cY), 3, (255, 0, 0), -1)
        cv2.putText(image, "center", (cX-10, cY-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
      for i in range(len(yellow_hits)):
        cX = yellow_hits[i][0]
        cY = yellow_hits[i][1]
        cv2.circle(image, (cX, cY), 3, (0, 255, 0), -1)
        cv2.putText(image, "center", (cX-10, cY-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
# integral part and aimpoint init
integralPart = 0
aimPoint = (0,0)
...
while True:
    ...
    canny_img = cv2.Canny(gray_img, 60, 120)
    ...
    blue_list, yellow_list, img = findCones(canny_img, hsv_low_blue, hsv_high_blue, hsv_low_yellow, hsv_high_yellow, hsv_img, img, cid)
