...
def findCones(blue_img, yellow_img, orange_img, image, cid):
  ...
  im2, cnts, hier = cv2.findContours(orange_img.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
  orange_hits = []
  for c in cnts:
    M = cv2.moments(c)
    cX = int(M["m10"] / M["m00"])
  ...
  return blue_hits, yellow_hits, orange_hits, image
...
while True:
    ...
    hsv_low_blue = (100, 98, 45) # for blue
    hsv_high_blue = (115, 255, 96) # for blue
    hsv_low_yellow = (25, 49, 155) # for yellow
    hsv_high_yellow = (33, 178, 216) # for yellow
    hsv_low_orange = (1, 100, 110) # for orange
    hsv_high_orange = (5, 160, 155) # for orange
    ...
    blue_list, yellow_list, orange_list, img = findCones(erode_blue, erode_yellow, erode_orange, img, cid)
